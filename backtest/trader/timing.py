...
def is_k_up_break_ma10(code, _date):
    ...
    # 从日行情数据库中根据数字货币简称和日期查询一条数据；
    current_daily = DB_CONN['daily'].find_one(
        {'code': code, 'date': _date})

    # 没有找到数字货币的日行情数据，则认为不符合日线收盘价上穿10日均线的条件；
    if current_daily is None:
        print('计算信号，K线上穿MA10，当日没有K线，数字货币 %s，日期：%s' % (code, _date), flush=True)
        return False

    # 从日行情数据集中查询11条数据，因为要计算连着两个交易日的10日均线价格，所以是需要11条数据；
    ...
    if len(dailies) < 11:
        print('计算信号，K线上穿MA10，前期K线不足，数字货币 %s，日期：%s' % (code, _date), flush=True)
        return False
    ...
    print('计算信号，K线上穿MA10，数字货币：%s，日期：%s， 前一日 %s，当日：%s，突破：%s' %
...
def is_k_down_break_ma10(code, _date):
    ...
    :return: True/False
    """
    # 从日行情数据集中根据数字货币简称和日期的条件查询一条数据；
    current_daily = DB_CONN['daily'].find_one(
        {'code': code, 'date': _date})

    # 没有找到数字货币的日行情数据，则认为不符合日线收盘价下穿10日均线的条件
    if current_daily is None:
        print('计算信号，K线下穿MA10，当日没有K线，数字货币 %s，日期：%s' % (code, _date), flush=True)
        return False
    ...
    if len(dailies) < 11:
        print('计算信号，K线下穿MA10，前期K线不足，数字货币 %s，日期：%s' % (code, _date), flush=True)
        return False
    ...
    print('计算信号，K线下穿MA10，数字货币：%s，日期：%s， 前一日 %s，当日：%s, 突破：%s' %
          (code, _date, str(last_close_2_last_ma10), str(current_close_2_current_ma10), str(is_break)), flush=True)
...
def compare_close_2_ma_10(dailies):
    ...
    # print('计算信号，数字货币： %s, 收盘价：%7.2f, MA10: %7.2f, 差值：%7.2f' %
